...
const {
  CreateTask, ListTasks, RemoveTask, UpdateTask,
...
module.exports.Create = async (req, res, next) => {
  try {
    ...
    const newTask = await CreateTask({
      text, status, userId, createAt,
    });
    res.status(ACCEPTED).json({ message: newTask });
  } catch (err) {
    next(err);
  }
};
